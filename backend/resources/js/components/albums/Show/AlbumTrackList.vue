  ...
  <DefaultSection class="p-6 space-y-5">
    ...
    <template v-if="Object.keys(discs).length > 0">
        ...
        <div v-for="(tracks, discNumber) in discs" :key="discNumber" class="space-y-2">
          ...
          <ol class="pl-5 list-decimal text-zinc-400 space-y-2 text-base">
            <li v-for="track in tracks" :key="track.id">
              ...
              <span v-if="track.length_formatted" class="text-zinc-400 font-sans text-sm"> ({{ track.length_formatted }})</span>
      ...
      <div class="text-zinc-400">
        Total length: <span class="font-bold text-zinc-50 font-sans"> {{ album.total_length_formatted }} </span>
      </div>
    </template>
    <template v-else>
