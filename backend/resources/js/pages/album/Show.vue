...
<template>
  <section class="container mx-auto rounded-lg flex p-10 gap-10 bg-zinc-850 border-2 border-zinc-800 shadow-xl">
    <div class="xl:h-120 xl:w-120 h-96 w-96 aspect-square object-center object-contain border-2 border-zinc-800 rounded-md overflow-hidden relative">
      <img v-if="album.cover_image_preview" :src="album.cover_image_preview" :alt="`${album.title} cover`" class="w-full h-full object-cover object-center">
      ...
      <div class="flex my-auto gap-10 border-2 p-5 rounded-md border-zinc-700">
        <div>
          ...
          <p class="text-zinc-300 my-2">
            <a v-if="album.artist" :href="route('artists.show', album.artist.slug)" class="hover:underline">{{ album.artist.name }}</a>
        ...
        <div :class="`flex items-center justify-center my-2 text-6xl mr-5 font-black leading-none ${album.rating_color}`">
